...
BASE_URL = ""  # not used — data is local (built by build_corpus.py)
MAX_SHARD = 2  # 3 shards: 0-2
VAL_SHARD = MAX_SHARD  # last shard is validation
...
def download_data(num_shards, download_workers=8):
    """Check that local shards exist (built by build_corpus.py)."""
    os.makedirs(DATA_DIR, exist_ok=True)
    ids = list(range(MAX_SHARD + 1))
    existing = sum(1 for i in ids if os.path.exists(os.path.join(DATA_DIR, f"shard_{i:05d}.parquet")))
    if existing == len(ids):
        print(f"Data: all {len(ids)} shards ready at {DATA_DIR}")
    else:
        print(f"Data: {existing}/{len(ids)} shards found. Run build_corpus.py first.")
        sys.exit(1)
